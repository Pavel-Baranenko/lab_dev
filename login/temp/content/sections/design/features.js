        ...
        function openFolder(list) {
          ...
          if (list.length > 0) {
            list.forEach((e, i) => {
              ...
              img.setAttribute('src', `/content/ressources/medias/${selectedFolder}/${e}`)
            })
          } else {
            ...
            socket.emit('checkAgencyStatus', userLSG, agencyStatus => {
              function serverHandler(userConfiguration) {
                ...
                destroyButton.addEventListener('click', () => {
                  const userLSG = lab_local_storage_object('global')
                  const vm_deletion_warning = lab_design_system_d('div', 'vm-warning', rootLayer, null, 'lab_modal')
                  vm_deletion_warning.style.zIndex = 999999
                  vm_deletion_warning.style.position = "fixed"
                  vm_deletion_warning.style.top = "15svh"
                  vm_deletion_warning.style.left = "25svw"
                  vm_deletion_warning.style.width = "50svw"
                  vm_deletion_warning.style.height = '50svh'
                  vm_deletion_warning.style.borderRadius = '50px'
                  vm_deletion_warning.style.color = "white"
                  vm_deletion_warning.style.backgroundColor = "rgb(60, 76, 166)"

                  const lab_account_deletion_exit = lab_design_system_d("img", "destroy-vm-settings-exit", vm_deletion_warning, null, 'lab_exit_black')
                  lab_account_deletion_exit.style.right = "30px"
                  lab_account_deletion_exit.addEventListener('click', () => vm_deletion_warning.remove())

                  const vm_deletion_warning_text = lab_design_system_d('div', 'vm-warning-text', vm_deletion_warning, lngData.warning_vm_supression, 'lab_form_label')
                  vm_deletion_warning_text.style.top = "10svh"
                  vm_deletion_warning_text.style.justifyContent = "center"
                  vm_deletion_warning_text.style.fontFamily = "Forum"
                  vm_deletion_warning_text.style.fontSize = "3svh"
                  vm_deletion_warning_text.style.width = ""

                  const vm_deletion_confirm = lab_design_system_d('div', 'vm-destroy-confirm', vm_deletion_warning, lngData.confirm, 'lab_button')
                  vm_deletion_confirm.style.position = "relative"
                  vm_deletion_confirm.style.borderRadius = "50px"
                  vm_deletion_confirm.style.backgroundColor = "#FCCE65"
                  vm_deletion_confirm.style.color = "black"
                  vm_deletion_confirm.style.userSelect = "none"
                  vm_deletion_confirm.style.fontFamily = "Forum"
                  vm_deletion_confirm.style.cursor = "pointer"

                  if (window.innerWidth >= window.innerHeight) {
                    vm_deletion_confirm.style.width = "30%"
                    vm_deletion_confirm.style.height = "5%"
                    vm_deletion_confirm.style.top = "40%"
                    vm_deletion_confirm.style.left = "35%"
                    vm_deletion_confirm.style.fontSize = "1.3svw"
                  } else {
                    vm_deletion_confirm.style.width = "30svw"
                    vm_deletion_confirm.style.height = "4%"
                    vm_deletion_confirm.style.left = "35%"
                    vm_deletion_confirm.style.top = "33svh"
                    vm_deletion_confirm.style.fontSize = "3svw"
                  }
                ...
                const outputRemoteCommand = lab_design_system_d('div', "cmd-output", leftBox, null, 'lab_form', 'lab_laboranth_scroll lab_deploy_menu')
                outputRemoteCommand.style.position = "absolute"
                outputRemoteCommand.style.backgroundColor = "black"
                outputRemoteCommand.style.color = "white"
                outputRemoteCommand.style.whiteSpace = "break-spaces"
                outputRemoteCommand.style.overflowY = "scroll"
                outputRemoteCommand.style.paddingLeft = "1%"
                outputRemoteCommand.style.borderRadius = "20px"
                outputRemoteCommand.style.fontFamily = "Forum"

                if (window.innerWidth >= window.innerHeight) {
                  outputRemoteCommand.style.width = "63%"
                  outputRemoteCommand.style.height = "86%"
                  outputRemoteCommand.style.top = "2%"
                  outputRemoteCommand.style.right = "1%"
                  outputRemoteCommand.style.left = ""
                } else {
                  outputRemoteCommand.style.width = "90%"
                  outputRemoteCommand.style.height = "85%"
                  outputRemoteCommand.style.top = "94%"
                  outputRemoteCommand.style.left = "5%"
                }

                const newRemoteCommand = lab_design_system_d('div', "new-remote-cmd", leftBox, null, 'lab_form', "lab_deploy_menu")
                newRemoteCommand.style.alignItems = "center"

                if (window.innerWidth >= window.innerHeight) {
                  newRemoteCommand.style.flexDirection = ""
                  newRemoteCommand.style.position = "absolute"
                  newRemoteCommand.style.top = "90%"
                  newRemoteCommand.style.right = "1.3%"
                  newRemoteCommand.style.left = ""
                  newRemoteCommand.style.height = "8%"
                  newRemoteCommand.style.width = "63%"
                } else {
                  newRemoteCommand.style.top = "15%"
                  newRemoteCommand.style.left = "5%"
                  newRemoteCommand.style.height = "17%"
                  newRemoteCommand.style.width = "90%"
                }

                const commandInput = lab_design_system_d('input', "cmd-input", newRemoteCommand, null, 'lab_text_input')
                commandInput.setAttribute('placeholder', "ls")
                commandInput.style.width = lab_orientation === "Landscape" ? 60 + "%" : 90 + "%"
                commandInput.style.height = lab_orientation === "Landscape" ? 45 + "%" : 25 + "%"
                commandInput.style.minHeight = "25px"
                commandInput.style.maxWidth = ""
                commandInput.style.paddingLeft = "2%"
                commandInput.style.border = "1px #808080 solid"
                commandInput.style.borderRadius = "50px"
                commandInput.style.minWidth = ""

                const runCommand = lab_design_system_d('div', "run-cmd", newRemoteCommand, lngData.run_cmd, 'lab_button')
                runCommand.style.backgroundColor = "rgb(60, 76, 166)"
                runCommand.style.color = "white"
                if (window.innerWidth >= window.innerHeight) {
                  runCommand.style.width = "20vw"
                  runCommand.style.height = "3svh"
                  runCommand.style.left = "20%"
                } else {
                  runCommand.style.width = "50%"
                  runCommand.style.height = "4svh"
                  runCommand.style.left = "25%"
                  runCommand.style.top = "20%"
                }

                runCommand.addEventListener('click', () => {
                  const userLSG = lab_local_storage_object('global')
                  userLSG.command = commandInput.value
                  userLSG.isAgencyServer = true
                  commandInput.value = ""
                  socket.emit("runRemoteCommand", userLSG)
                })

                if (window.innerWidth <= window.innerHeight) {
                  const lab_end_deploy_spacer = lab_design_system_d('div', "deploy-spacer", leftBox, null, 'lab_container')

                  lab_end_deploy_spacer.style.position = "relative"
                  lab_end_deploy_spacer.style.width = "100%"
                  lab_end_deploy_spacer.style.height = "3%"
                  lab_end_deploy_spacer.style.top = "105%"
                }
              }
                  ...
                  if (userConfiguration.configs.agencyServer) {
                    ...
                    initialize.addEventListener('click', (e) => {
                      ...
                      if (userNameInput.value && userPassInput.value && isValidPassword(userPassInput.value)) {
                        ...
                        spinner.setAttribute('src', '/content/sections/design/img/pre-loader.svg')
  ...
  async function processFile() {
    try {
      ...
      await socket.emit('droppedImages', userLSG, async res => {
        items.forEach(e => {
          e.setAttribute('src', `/${res.mediaType}/${res.id}.webp`)
          e.id = res.id
